        ...
        if width == 0 {
            if pos.col > 0 {
                let mut prev_cell = self
                    ...
                    .unwrap();
                // cys 패치(2026-07-16): pos.col==1에서 col-2는 u16 언더플로 OOB(좁은 resize로 와이드
                // 선두가 그리드 밖·연속 셀만 남은 극단). col>1일 때만 선두(col-2)로 재지정한다 —
                // 아니면 연속 셀에 append(무-패닉·희소 combining 경로).
                if prev_cell.is_wide_continuation() && pos.col > 1 {
                    prev_cell = self
                        .grid_mut()
                        .drawing_cell_mut(crate::grid::Pos {
                            row: pos.row,
            ...
            {
                // cys 패치(2026-07-16): "wide 연속 셀이면 col-1에 선두가 반드시 있다" 가정은 좁은
                // resize로 선두가 그리드 밖으로 밀리면 깨진다 — col==0이면 col-1이 u16 언더플로,
                // 아니어도 drawing_cell_mut None → unwrap 패닉. 선두가 없으면 무동작(경계 안전).
                if pos.col > 0 {
            ...
            if self
                ...
                .is_wide()
            {
                // cys 패치(2026-07-16): 위 주석의 "wide면 col+1에 후반 셀이 반드시 있다" 가정은
                // 좁은 폭으로의 resize로 와이드 선두 셀이 마지막 열에 놓이면 깨진다 — col+1이 그리드
                // 밖이라 drawing_cell_mut이 None → unwrap 패닉(clear_wide OOB와 동일 근본 클래스).
                // off-grid면 지울 후반 셀이 없으므로 무동작(경계검사만·정상 경로 동작 불변).
            ...
            if width > 1 {
                    ...
                    .is_wide()
                {
                    ...
                    // cys 패치(2026-07-16): 위 주석의 "wide면 col+1에 후반 셀이 있다" 가정은 좁은
                    // resize로 와이드 셀이 마지막 열에 놓이면 깨진다 — col+1 off-grid면
                    // drawing_cell_mut None → unwrap 패닉. off-grid면 지울 후반 셀이 없어 무동작.
                    // (차용을 clear까지로 좁혀 아래 drawing_row_mut와 충돌 회피.)
