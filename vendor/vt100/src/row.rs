...
impl Row {
    ...
    pub fn clear_wide(&mut self, col: u16) {
        ...
        let other = if cell.is_wide() {
            // cys 패치(2026-07-16): 와이드(CJK) 선두 셀이 좁은 폭으로의 resize로 마지막 열에 놓이면
            // 연속 셀(col+1)이 그리드 밖이다 — 무검사 인덱싱은 panic("len N index N", 구 row.rs:89).
            // off-grid면 지울 연속 셀이 없으므로 무동작한다(경계검사만 추가·그 외 동작 불변).
            ...
            // cys 패치: col==0이면 col-1이 u16 언더플로로 OOB — 선두 셀이 없으므로 무동작.
            if col == 0 {
                return;
            }
