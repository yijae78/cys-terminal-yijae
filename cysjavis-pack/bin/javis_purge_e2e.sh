...
echo "== 7) javis_org destroy --purge-state (state+pack+workdir 동일 trash) =="
setup_depts
...
OUT9=$(python3 "$ORG" destroy --dept testdept --purge --purge-workdir --purge-state 2>&1) && RC9=0 || RC9=$?
...
STUBDEPT2
chmod +x "$T/.local/bin/cys-dept"
OUT10=$(python3 "$ORG" destroy --dept testdept --purge --purge-workdir --purge-state 2>&1) && RC10=0 || RC10=$?
rm -f "$T/.local/bin/cys-dept"
